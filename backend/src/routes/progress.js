...
router.get('/:userId', (req, res) => {
  try {
    ...
    let userProgress = mockProgress.filter(progress => progress.userId === userId);
    ...
    const totalHours = userProgress.reduce((sum, progress) => sum + progress.hours, 0);
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          ...
          averageHoursPerSession: totalHours / userProgress.length || 0
        }
      }
    });
  } catch (error) {
