...
router.get("/", (req, res) => {
    try {
        ...
        let filteredPartners = mockPartners.filter(
            (partner) => partner.id !== req.user.uid,
        );
        ...
        if (notModule) {
            const excludeModules = notModule
                .split(",")
                .map((m) => m.trim().toUpperCase());
        ...
        res.json({
            ...
            data: {
                ...
                totalPages: Math.ceil(
                    filteredPartners.length / parseInt(limit),
                ),
            },
        });
    } catch (error) {
