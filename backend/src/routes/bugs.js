    ...
    async (req, res) => {
        try {
            ...
            const bugData = {
                ...
                reporterId,
                reporterName:
                    reporterData.displayName || reporterData.email || "Unknown",
...
router.get("/", async (req, res) => {
    try {
        ...
        let bugs = snapshot.docs.map((doc) => ({
            ...
            resolvedAt:
                doc.data().resolvedAt?.toDate?.()?.toISOString() || null,
        }));
...
router.get("/:id", async (req, res) => {
    try {
        ...
        const bugDoc = await admin.firestore().collection("bugs").doc(id).get();
...
router.put(
    ...
    [
        ...
        body("category")
            .optional()
            .isIn(["ui", "api", "performance", "security", "data", "other"]),
        body("status")
            .optional()
        ...
        try {
            ...
            if (!isReporter && !isAssignee) {
                return res.status(403).json({
                    error: "Access denied",
                    message:
                        "You do not have permission to update this bug report",
                });
            }
            ...
            if (
                ...
                updateData.assignedTo !== bugData.assignedTo
            ) {
                const assigneeDoc = await admin
                    .firestore()
                    .collection("users")
                ...
                updates.assignedToName = assigneeDoc.exists
                    ? assigneeDoc.data().displayName ||
                      assigneeDoc.data().email ||
                      "Unknown"
                    : "Unknown";
            }

            // Update bug report
            await admin.firestore().collection("bugs").doc(id).update(updates);
            ...
            const updatedBugDoc = await admin
                .firestore()
                .collection("bugs")
                .doc(id)
                .get();
            ...
            res.json({
                ...
                bug: {
                    ...
                    ...updatedBugData,
                    createdAt:
                        updatedBugData.createdAt?.toDate?.()?.toISOString() ||
                        null,
                    updatedAt:
                        updatedBugData.updatedAt?.toDate?.()?.toISOString() ||
                        null,
                    resolvedAt:
                        updatedBugData.resolvedAt?.toDate?.()?.toISOString() ||
                        null,
                },
    ...
    async (req, res) => {
        try {
            ...
            await admin
                ...
                .update({
                    comments:
                        admin.firestore.FieldValue.arrayUnion(commentData),
                    updatedAt: admin.firestore.FieldValue.serverTimestamp(),
                });
...
router.post("/:id/watch", async (req, res) => {
    try {
        ...
        const bugData = bugDoc.data();
        const isWatching =
            bugData.watchers && bugData.watchers.includes(userId);
...
router.get("/stats", async (req, res) => {
    try {
        const bugsSnapshot = await admin.firestore().collection("bugs").get();
        ...
        const stats = {
            ...
            byStatus: {
                ...
                duplicate: bugs.filter((bug) => bug.status === "duplicate")
                    .length,
            },
            bySeverity: {
                ...
                critical: bugs.filter((bug) => bug.severity === "critical")
                    .length,
            },
            byCategory: {
                ui: bugs.filter((bug) => bug.category === "ui").length,
                api: bugs.filter((bug) => bug.category === "api").length,
                performance: bugs.filter(
                    (bug) => bug.category === "performance",
                ).length,
                security: bugs.filter((bug) => bug.category === "security")
                    .length,
