...
router.get('/', async (req, res) => {
  try {
    ...
    // Check cache first
    const cacheDuration = parseInt(process.env.WEATHER_CACHE_DURATION) || 300000; // 5 minutes default
    const now = Date.now();
...
router.post('/recommendations', [
  body('studyType').optional().isIn(['indoor', 'outdoor', 'group', 'individual']),
  ...
  try {
    ...
    // Get current weather
    const weatherResponse = await axios.get(`${req.protocol}://${req.get('host')}/api/weather`);
    const weather = weatherResponse.data;

    const recommendations = generateStudyRecommendations(weather, studyType, duration);
...
function generateStudyRecommendations(weather, studyType, duration) {
  ...
  if (studyType === 'outdoor' || studyType === 'group') {
    if (temp >= 18 && temp <= 25 && windSpeed < 10 && !description.includes('rain')) {
      recommendations.location.push({
        type: 'outdoor',
        places: ['Library courtyard', 'Central campus lawn', 'Student center outdoor area'],
  ...
  if (studyType === 'group') {
    recommendations.activities.push('Group discussions', 'Peer teaching', 'Study games');
  } else {
    recommendations.activities.push('Reading', 'Note-taking', 'Problem solving');
  }

  // Weather-specific tips
  if (humidity > 70) {
    recommendations.tips.push('High humidity - stay hydrated and take breaks');
  }
  if (windSpeed > 10) {
    recommendations.tips.push('Windy conditions - secure your study materials');
  }
  if (description.includes('sun')) {
    recommendations.tips.push('Sunny weather - use sunscreen for outdoor study');
  }
