...
// export default GroupChatView;import React, { useState, useEffect, useRef } from "react";
...
// //this is a code from my other project and it works. 
// const uploadFileToFirebase = async (file) => {
//   if (!file) {
//     throw new Error('No file provided');
//   }

//   toast.loading('Starting file upload...', { id: 'file-upload' });
//   setUploading(true);
//   setProgress(0);
...
//         toast.loading(`Uploading: ${Math.round(prog)}%`, { id: 'file-upload' });
//       },
//       (error) => {
...
//           setUploading(false);
//           setProgress(0);
//           toast.error('Failed to get download URL', { id: 'file-upload' });
//           reject(error);
//         }
//       }
//     );
...
//           fileData = await uploadFileToFirebase(selectedFile, `study-groups/${group.id}`);
...
//           body: JSON.stringify({
//             userId: currentUser.uid,
//             message: newMessage.trim() || (fileData ? fileData.originalName : ''),
...
//                       <span className="text-sm truncate block">{attachment.name || attachment}</span>
//                       {attachment.size && (
...
const GroupChatView = ({ group, onBack, currentUser, baseUrl }) => {
  ...
  // Fixed uploadFileToFirebase function - using uploadBytes instead of uploadBytesResumable
  const uploadFileToFirebase = async (file) => {
    if (!file) {
      throw new Error('No file provided');
    }
    ...
    try {
      ...
      // Use simple uploadBytes instead of uploadBytesResumable for better CORS compatibility
