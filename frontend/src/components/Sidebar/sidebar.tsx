  ...
  return (
    <>
      ...
      <button
        ...
        className="md:hidden fixed top-4 left-4 z-50 flex flex-col justify-center items-center space-y-1.5 w-10 h-10 rounded-lg  transition-colors"
        aria-label="Toggle menu"
      >
        <div
          className={`h-0.5 w-6 bg-slate-900 rounded transition-all duration-300 ${
            isMobileOpen ? "rotate-45 translate-y-2 w-7 bg-white hidden" : "w-5"
          }`}
        />
        <div
        ...
        <div
          className={`h-0.5 bg-slate-900 rounded transition-all duration-300 ${
            isMobileOpen
              ? "-rotate-45 -translate-y-2 w-7 bg-white hidden"
              : "w-4"
          }`}
        />
      </button>
