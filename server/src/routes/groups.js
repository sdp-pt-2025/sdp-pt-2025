...
const mockGroups = [
  ...
  {
    id: '2',
    name: 'MATH2001 Problem Solvers',
    description: 'Mathematics for Computer Science study group',
    module: 'MATH2001',
    maxMembers: 6,
    currentMembers: 4,
    members: [
      { id: 'user1', name: 'Alice Johnson', role: 'member' },
      { id: 'user3', name: 'Carol Davis', role: 'admin' },
      { id: 'user6', name: 'Frank Miller', role: 'member' },
      { id: 'user7', name: 'Grace Lee', role: 'member' }
    ],
    schedule: ['Tuesday 1-3pm', 'Thursday 2-4pm'],
...
const mockMessages = {
  1: [
    ...
    },
    {
      ...
      userName: 'Bob Smith',
      message:
        "Yes! I've been reviewing the material. Should we focus on the design patterns chapter?",
  ...
  2: [
    {
      ...
      message: 'Math group meeting today at 2pm. Bring your calculators!',
 ...
 * @access  Private
 */
router.post('/', verifyToken, validateCreateGroup, (req, res) => {
  try {
    const { name, description, module, maxMembers, schedule, location } =
      req.body;
  ...
  try {
    ...
    if (moduleFilter) {
      filteredGroups = filteredGroups.filter(
        group => group.module === moduleFilter.toUpperCase()
      );
    }
 ...
 * @desc    Get a specific study group by ID
 * @access  Private
 */
router.get('/:id', verifyToken, validateGroupId, (req, res) => {
  try {
    const { id } = req.params;
 ...
 * @access  Private
 */
router.post('/:id/join', verifyToken, validateGroupId, (req, res) => {
  try {
    const { id } = req.params;
    const group = mockGroups.find(g => g.id === id);
 ...
 * @access  Private
 */
router.get('/:id/messages', verifyToken, validateGroupId, (req, res) => {
  try {
    const { id } = req.params;
